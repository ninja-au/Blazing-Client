...
public class ToggleSneakMod extends Mod implements Renderable {
    ...
    public void render() {
        ...
        String Component;
        if(backgroundEnabled.isEnabled()) Component = "Sneak: " + (isSneaking() ? "Toggled" : "Vanilla");
        else Component = "[Sneak: " + (isSneaking() ? "Toggled]" : "Vanilla]");
    ...
    public void onKey(KeyPressEvent e) {
        if (InputConstants.isKeyPressed(Minecraft.getInstance().getWindow().getHandle(), mc.options.sneakKey.getDefaultKey().getCode())) {
            toggled = !toggled;
        }
    }
}
